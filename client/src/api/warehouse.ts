...
// ============================================================
// Basic Warehouse type (exported for use in other modules)
// ============================================================
...
export async function getDashboard() {
  const response = await api.get<WarehouseDashboardResponse>("/warehouses/dashboard");
  return response.data;
}

export async function getWarehouses(page = 1, search = "") {
  const response = await api.get("/warehouses", {
    params: { page, search },
  });
  // response.data should be { data: Warehouse[], pagination: {...} }
  return response.data;
}

export async function getWarehouse(id: number) {
  const response = await api.get<WarehouseDetails>(`/warehouses/${id}`);
  return response.data;
}
...
export async function activateWarehouse(id: number) {
  return api.patch(`/warehouses/${id}/activate`);
}

export async function deactivateWarehouse(id: number) {
  return api.patch(`/warehouses/${id}/deactivate`);
}
...
export async function restoreWarehouse(id: number) {
  return api.patch(`/warehouses/${id}/restore`);
}
