...
export async function getSalesReport(filters: ReportFilters) {
  ...
  const productTotals: Record<number, {
    name: string;
    revenue: number;
    quantity: number;
    cost: number;
  ...
  const productMonthly: Record<number, { month: string; revenue: number }[]> = {};
  ...
  const dayTotals: Record<number, { day: string; revenue: number; orders: number }> = {};
  const monthTotals: Record<number, { month: string; revenue: number; orders: number }> = {};
  ...
  const warehouseSales: Record<number, {
    name: string;
    revenue: number;
    profit: number;
    orders: number;
...
export async function loadSavedReport(reportId: number, userId: number) {
  const report = await prisma.report.findUnique({
    where: { id: reportId },
    include: { results: true },
  });
  if (!report) throw new Error("Report not found.");
  if (report.createdById !== userId) throw new Error("Unauthorized.");
  if (report.results) return report.results.data;
  const filters = report.filters as any as ReportFilters;
  const data = await getSalesReport(filters);
...
export async function getInventoryReport(filters: {
  ...
  products.forEach((p) => {
    ...
    if (stock < Number(p.minimumStock) && stock > 0) lowStockCount++;
  ...
  const salesQuantityByProduct: Record<number, number> = {};
  ...
  const avgDailySales: Record<number, number> = {};
  ...
  const monthlyTrend: Record<string, { month: string; value: number }> = {};
